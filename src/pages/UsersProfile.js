...
getUser();
}, [userId, history, addToast])   // this is dependency array ,userId is passed because if userId changes , we need to refresh the user details
...
const handleRemoveFriendClick = async() => {
    ...
    const response = await removeFriend(userId);
        if(response.success){
             const friendship =  auth.user.friends.filter(       // response object mei nhi aayenge user k friends
           (friend) => friend.to_user._id === userId);

             auth.updateUserFriends(false, friendship[0]);            // because filter returns an array
